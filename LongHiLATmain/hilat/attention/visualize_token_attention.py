...
# read raw data
with open('C:/Users/mkanneganti/Downloads/new/data/mimic3/50/test_data_50_level_1_new_8_seq-512.pkl', 'rb') as f:
    jls_extract_var = pickle
    test_data = jls_extract_var.load(f)

# read prediction results
with open('C:/Users/mkanneganti/Downloads/new/model/predict_results_mimic3-50.pkl', 'rb') as f:
...
def display_attention(data, results, code_dict, tokenizer, record_no):
    tokens_attention = results['predictions'][1][record_no]
    chunk_attention = results['predictions'][2][record_no]
    true_labels = results['labels'][record_no]
    pred_labels = np.rint(results['predictions'][0][record_no])
