...
sys.path.append("C:/Users/mkanneganti/Downloads/new")
...
from LongLAT.hilat.models.modeling import CodingModel, CodingModelConfig
from LongLAT.hilat.models.utils import initial_code_title_vectors, calculate_scores, tokenize_dataset, MimicIIIDataset, \
...
# Set the logging level for the trainer progress logs
...
def main():
        ...
        if not data_args.use_cached_datasets:
            ...
            if data_args.data_segmented:
                text = data.loc[:, data.columns.str.startswith("Chunk")].fillna("").apply(
                    ...
                    axis=1).tolist()
                labels = data.iloc[:, 9:].apply(lambda x: [seg for seg in x], axis=1).tolist()
                results = tokenize_dataset(tokenizer, text, labels, data_args.max_seq_length)
            else:
                text = data["text"].tolist()
                import ast
                labels = data.iloc[:, 2:].apply(lambda x: [seg for seg in x], axis=1).tolist()
    ...
    model.save_pretrained("Long-LAT_5")
